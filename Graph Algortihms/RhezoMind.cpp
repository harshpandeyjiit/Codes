...
void dfs(int v, int p = -1)
{
    ...
    for (int to : adj[v])
    {
        ...
        else
        {
            ...
            if (low[to] >= tin[v] && p!=-1)artpt[v]=1;
            ++children;
        }
    }
    if(p == -1 && children > 1)artpt[v]=1;
}
...
int32_t main()
{
    ...
    while(t--)
    {
        ...
        memset(artpt,0,sizeof(indegree));
        cin>>n>>m;
        rep(i,m)
        {
            int a,b;
        ...
        while(q--)
        {
            int x;
            cin>>x;
            if(artpt[x])cout<<"Satisfied "<<indegree[x]<<endl;
