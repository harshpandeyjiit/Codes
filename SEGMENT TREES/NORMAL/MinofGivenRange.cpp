...
int minimum(int v,int tl,int tr,int l,int r)
{
	...
	int tm=tl+(tr-tl)/2;
	return min(minimum(v*2+1, tl, tm, l, min(r, tm)),minimum(v*2+2, tm+1, tr, max(l, tm+1), r));
}
...
int32_t main()
{
    ...
    while(test_case--)
    {
		...
		rep(i,n)cin>>arr[i];
		build(arr,0,0,n-1);
		//rep(i,2*n+1)cout<<t[i]<<" ";//print the build tree
		int q;
		cin>>q;
		rep(i,q)
		{
			int a,b;
			cin>>a>>b;
			cout<<minimum(0,0,n-1,a-1,b-1)<<endl;
		}
    }
    return 0;
}
